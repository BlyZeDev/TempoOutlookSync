...
namespace TempoOutlookSync
{
    public sealed class OutlookClient : IDisposable
    {
        ...
        public Dictionary<string, HashSet<AppointmentItem>> GetOutlookTempoAppointments(DateTime start)
        {
            ...
            items = items.Restrict($"@SQL=\"http://schemas.microsoft.com/mapi/string/{{00020329-0000-0000-C000-000000000046}}/{OutlookTempoIdProperty}\" IS NOT NULL AND \"http://schemas.microsoft.com/mapi/proptag/0x00610040\" >= '{start:yyyy-MM-dd HH:mm}'");
            items.Sort("[Start]");
            ...
            return items.OfType<AppointmentItem>()
                ...
                .Where(x => x.TempoId?.Value != null)
                .GroupBy(x => Convert.ToString(x.TempoId.Value) as string)
